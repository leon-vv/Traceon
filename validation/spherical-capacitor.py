...
def create_geometry(N, symmetry, for_plot):
    ...
    with occ.Geometry() as geom:
        ...
        lcar = r2/N
        points = [geom.add_point([p[0], 0, p[1]] if symmetry=='3d' else p, lcar) for p in points]
        center = geom.add_point([0, 0, 0], lcar)
...
util.parse_validation_args(create_geometry, compute_error,
    N={'radial':[10, 50, 100, 300,500,700],'3d':[5,8,10,12,14,16]},
