...
class TwoCylinderEdwards(Validation):
    ...
    def create_mesh(self, MSF, symmetry, higher_order):
        ...
        bottom  = v.Path.line([0., 0., 0.], [R, 0., 0.]).extend_with_line([R, 0., cylinder_length])
        gap = v.Path.line([R, 0., cylinder_length], [R, 0., cylinder_length+gap_size])
        top = v.Path.line([R, 0., cylinder_length+gap_size], [R, 0., boundary_length]).extend_with_line([0., 0., boundary_length])
    ...
    def get_excitation(self, geom, symmetry):
        exc = v.Excitation(geom, symmetry)
