...
class Path(GeometricObject):
    ...
    def map_points(self, fun: Callable[[PointLike3D], Point3D]) -> Path:
        ...
        Path"""
        return Path(lambda u: fun(self(u)), self.parameter_max, self.breakpoints, name=self.name)
    ...
    def add_phase(self, l: float) -> Path:
        ...
        return Path(fun, self.parameter_max, sorted([(b-l)%self.parameter_max for b in self.breakpoints + [0.]]), name=self.name)
