...
Point3D = tuple[float, float , float] | list[float] | NDArray[np.floating]
Point3DArray = NDArray[np.floating]

Vector3D = tuple[float, float , float] | list[float] | NDArray[np.floating]
Vector3DArray = NDArray[np.floating]
...
def _points_close(p1: Point3D, p2: Point3D, tolerance: float = 1e-8) -> bool:
...
class Path(GeometricObject):
    ...
    def __init__(
            self, 
            fun: Callable[[float], Point3DArray], 
            path_length: float, 
    ...
    @staticmethod
    def from_irregular_function(to_point: Callable[[float], Point3DArray], N: int = 100, breakpoints: list[float] | None = None) -> Path:
    ...
    def map_points(self, fun: Callable[[Point3D], Point3DArray]):
    ...
    def __call__(self, t: float) -> Point3DArray:
    ...
    def starting_point(self) -> Point3DArray:
    ...
    def middle_point(self) -> Point3DArray:
    ...
    def endpoint(self) -> Point3DArray:
    ...
    def extend_with_line(self, point: Point3D) -> Path:
    ...
    @staticmethod
    def arc(center: Point3D, start: Point3D, end: Point3D, reverse: bool = False) -> Path:
    ...
    def extrude(self, vector: Vector3D) -> Surface:
    ...
    @staticmethod
    def line(from_: Point3D, to: Point3D) -> Path:
    ...
    @staticmethod
    def polar_arc(radius: float, angle: float, start: Point3D, direction: Vector3D, plane_normal: Vector3D = (0,1,0)) -> Path:
    ...
    def extend_with_polar_arc(self, radius: float, angle: float, plane_normal: Vector3D = (0,1,0)) -> Path:
    ...
    def velocity_vector(self, t: float) -> Vector3DArray:
    ...
    def mesh(self, mesh_size: float | None = None, mesh_size_factor: float | None = None, higher_order: bool = False, name: str | None = None, ensure_outward_normals: bool = True) -> Mesh:
...
class PathCollection(GeometricObject):
    ...
    def map_points(self, fun: Callable[[Point3D], Point3DArray]) -> PathCollection:
    ...
    def extrude(self, vector: Vector3D) -> SurfaceCollection:
...
class Surface(GeometricObject):
    ...
    def __init__(
            self, 
            fun: Callable[[float, float], Point3DArray], 
            path_length1: float, 
            path_length2: float, 
    ...
    def __call__(self, u: float, v: float) -> Point3DArray:
    ...
    def map_points(self, fun: Callable[[Point3D], Point3DArray]) -> Surface:
    ...
    @staticmethod
    def box(p0: Point3D, p1: Point3D) -> SurfaceCollection:
    ...
    def extrude_boundary(self, vector: Vector3D, enclose: bool = True) -> SurfaceCollection:
        """
    ...
    def mesh(self, mesh_size: float | None = None, mesh_size_factor: float | None = None, name: str | None = None, ensure_outward_normals: bool = True) -> Mesh:
...
class SurfaceCollection(GeometricObject):
    ...
    def map_points(self, fun: Callable[[Point3D], Point3DArray]) -> SurfaceCollection:
